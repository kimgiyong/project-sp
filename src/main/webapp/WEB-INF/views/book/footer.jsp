<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<link rel="stylesheet" type="text/css" href="${resPath}/css/footer.css">
<footer>
	<div class="logoBox" id="foologoBox">
		<img src="${resPath}/img/logo.PNG" alt="downlogo">
	</div>

	<div class="footer-lf-tp">
		<ul class="clearfix">
			<li><a 
				 onclick="window.open('/uri/book/popup/privacyPage2','','width=1260,height=600,top=100,left=300');">개인정보처리방침</a></li>
			<li class="buga">|</li>
			<li><a onclick="window.open('/uri/book/popup/emailNotFound','','width=800,height=130,top=300,left=200');">이메일수집거부정책</a></li>
			<li class="buga">|</li>
			<li><a onclick="window.open('/uri/book/popup/copyrightPop','','width=800,height=450,top=80,left=250');">저작권정책</a></li>
			<li class="buga">|</li>
			<li><a href="/uri/book/menu/infoUse">이용안내</a></li>
			<li class="buga">|</li>
			<li><a href="/uri/book/menu/qna/qnaList">Q & A</a></li>
			<li class="buga">|</li>
			<li><a href="/uri/book/menu/mapinfo">찾아오시는길</a></li>
		</ul>
	</div>
	<pre>
		
		
		
도서관명 : 좋은향 도서관  |  설립일 : 2018.12.05  |  대표 : 김기용,한승우,Mr.agi  |  이사장 : 조은향


도서관 주소 : 서울시 금천구 가산디지털1로 186. (가산동. 제이플라츠 5F. 515,516호)


연락처 : 02)653-1234,335-2523 fax.031-228-2252 / admin@agi.net
		</pre>
</footer>